...
- **fal.ai** (агрегатор, pay-as-you-go) — одна вкладка, внутри выпадашка из ~10 актуальных моделей: **Kling V3 Pro/Standard, Seedance 2.0 (+Fast), Google Veo 3.1 (+Fast), Sora 2 Pro, PixVerse V6, Grok Imagine 1.5 (xAI), Wan 2.7**. Оплата картой по факту.
...
3. Вкладка **fal.ai** → выпадашка «Модель fal» (~10 актуальных, июль 2026): Kling V3 Pro/Standard, Seedance 2.0 (+Fast),
   Veo 3.1 (+Fast), Sora 2 Pro, PixVerse V6, Grok Imagine 1.5 (xAI), Wan 2.7.
...
- Очередь fal: `POST https://queue.fal.run/{endpoint}` (заголовок `Authorization: Key …`) → `response_url` → поллинг
  `…/status` → результат, видео в `video.url`. Схемы всех 10 моделей выверены по **живым OpenAPI-схемам fal** (эндпоинты, поля, длительности, цены).
- Нюансы схем, которые уже учтены: у Kling V3 картинка идёт в `start_image_url` (не `image_url`); Seedance 2.0 и Grok — **без**
  префикса `fal-ai/` в эндпоинте (`bytedance/…`, `xai/…`); Veo 3.1 хочет длительность строкой `"8s"`; у части i2v-эндпоинтов
  (Kling, Grok, PixVerse, Wan) нет `aspect_ratio` — формат задаёт картинка (`input.aspectI2V:false`). Цены — по тарифам fal,
  зависят от разрешения (`pricing.perSecond.byRes`) и звука (`{on,off}` / `secAudio`); списывает fal по факту.
...
> Разные подмодели тарифицируются по-разному (PixVerse от $0.025/с … Seedance 2.0 в 4k $1.56/с) — цифры в UI это оценка, списывает fal по факту.

## Актуальность моделей (встроенная проверка)

Каталоги видеомоделей меняются каждый месяц, поэтому в приложение встроена сверка конфига с живыми API:
- **Кнопка «🛰 Модели»** в шапке — панель с отчётом по каждой модели: ✅ актуальна / ⚠️ настройки разошлись со схемой /
  🆕 вышла новее / ⛔ депрекация или эндпоинт пропал. Раз в сутки проверка тихо запускается сама и подсвечивает кнопку, если есть находки.
- **CLI: `npm run check-models`** — тот же отчёт в терминале (и HTTP: `GET /api/check-updates`).
- Что проверяется: для каждого fal-эндпоинта (t2v и i2v) скачивается его OpenAPI-схема (`fal.ai/api/openapi/queue/openapi.json?endpoint_id=…`)
  и сверяется с config.json — диапазоны длительности, списки разрешений и форматов, наличие полей (звук/негатив/cfg/seed/картинки);
  по каталогу (`fal.ai/api/models?keywords=…`) ловятся флаги deprecated/removed, актуальные цены и более новые версии семейства.
  Для Google проверяется, что модель Omni жива в API. BytePlus публичного каталога не отдаёт — панель напоминает сверить в консоли.
